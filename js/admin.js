// ==========================================
// ADMIN PANEL - FINAL VERSION WITH ALL FIXES
// All buttons working, mobile friendly
// ==========================================
...
// ==========================================
// EXPORT FUNCTIONS FIRST - CRITICAL!
// ==========================================
...
window.adminPanel = {
  ...
  showAddForm: function(type) {
    showAddForm(type);
  },
  editItem: function(type, item) {
    editItem(type, item);
  },
  ...
  submitForm: function(event, type, isEdit) {
    submitForm(event, type, isEdit);
  }
};
...
function initializeAdmin() {
  ...
  // Setup listeners after HTML is inserted
  setTimeout(() => {
    setupAdminListeners();
    loadTab('articles');
  }, 100);
      ...
      <div class="container mx-auto px-2 md:px-4 py-4 md:py-8">
        ...
        <div class="bg-white rounded-lg shadow mb-4 md:mb-6 overflow-x-auto">
          <div class="flex border-b min-w-max md:min-w-0">
            <button class="admin-tab px-3 md:px-6 py-3 md:py-4 font-semibold text-sm md:text-base whitespace-nowrap" 
              data-tab="articles">
            ...
            ${articles.map(article => `
              <tr class="border-b hover:bg-gray-50">
                ...
                <td class="px-2 md:px-4 py-2 md:py-3 text-right">
                  <button onclick='window.adminPanel.editItem("article", ${JSON.stringify(article).replace(/'/g, "\\'").replace(/"/g, '&quot;')})'
            ...
            <div class="bg-white border rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div class="p-3 md:p-4">
                ...
                <div class="flex gap-2">
                  <button onclick='window.adminPanel.editItem("product", ${safeProduct})'
  ...
  try {
    ...
    container.innerHTML = `
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
        ${ads.map(ad => {
          const safeAd = JSON.stringify(ad).replace(/'/g, "\\'").replace(/"/g, '&quot;');
          return `
            <div class="bg-white border rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <div class="p-3 md:p-4">
                ...
                <div class="flex gap-2">
                  <button onclick='window.adminPanel.editItem("ad", ${safeAd})'
                    class="flex-1 bg-blue-500 text-white px-2 md:px-3 py-1 md:py-2 rounded hover:bg-blue-600 text-xs md:text-sm">
                    Éditer
                  </button>
...
// ==========================================
// FORM TEMPLATES - Same as before
// (Keeping getArticleForm, getProductForm, getAdForm the same)
// ==========================================
    ...
    <form id="adminForm" class="space-y-4" onsubmit="window.adminPanel.submitForm(event, 'article', ${isEdit})">
      <div>
        ...
        <input type="text" name="title" value="${item.title || ''}" required
          class="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-pink-500">
      </div>
      ...
      <div>
        ...
        <textarea name="excerpt" rows="3" required class="w-full px-3 py-2 border rounded">${item.excerpt || ''}</textarea>
      </div>
      
      <div>
        <label class="block text-sm font-medium mb-1">Contenu *</label>
        <textarea name="content" rows="6" required class="w-full px-3 py-2 border rounded">${item.content || ''}</textarea>
      </div>
      
      <div>
        <label class="block text-sm font-medium mb-1">Images</label>
        <input type="file" name="images" multiple accept="image/*" class="w-full px-3 py-2 border rounded">
      </div>
      ...
      <div class="flex flex-col-reverse md:flex-row gap-3 pt-4">
        <button type="button" onclick="window.adminPanel.closeModal()"
          class="px-6 py-3 border rounded hover:bg-gray-50">
          Annuler
        </button>
        <button type="submit" class="px-6 py-3 bg-pink-500 text-white rounded hover:bg-pink-600">
  ...
  return `
    <form id="adminForm" class="space-y-4" onsubmit="window.adminPanel.submitForm(event, 'product', ${isEdit})">
      ...
      <div>
        ...
        <textarea name="description" rows="3" required class="w-full px-3 py-2 border rounded">${item.description || ''}</textarea>
      </div>
      
      <div>
        <label class="block text-sm font-medium mb-1">Prix (DZD) *</label>
        <input type="number" name="price" value="${item.price || ''}" required min="0" step="0.01" class="w-full px-3 py-2 border rounded">
      </div>
      
      <div>
        <label class="block text-sm font-medium mb-1">Catégorie *</label>
        <input type="text" name="category" value="${item.category || ''}" required class="w-full px-3 py-2 border rounded">
      </div>
      
      <div>
        <label class="block text-sm font-medium mb-1">Quantité en stock</label>
        <input type="number" name="stockQuantity" value="${item.stockQuantity || 0}" min="0" class="w-full px-3 py-2 border rounded">
      </div>
      ...
      <div class="flex flex-col-reverse md:flex-row gap-3 pt-4">
        <button type="button" onclick="window.adminPanel.closeModal()" class="px-6 py-3 border rounded">Annuler</button>
        <button type="submit" class="px-6 py-3 bg-pink-500 text-white rounded hover:bg-pink-600">
          ${isEdit ? 'Mettre à jour' : 'Créer'}
        </button>
      </div>
...
function getAdForm(item = {}, isEdit = false) {
  return `
    <form id="adminForm" class="space-y-4" onsubmit="window.adminPanel.submitForm(event, 'ad', ${isEdit})">
      <div>
        <label class="block text-sm font-medium mb-1">Contenu *</label>
        <textarea name="content" rows="4" required class="w-full px-3 py-2 border rounded">${item.content || ''}</textarea>
      </div>
      
      <div>
        <label class="block text-sm font-medium mb-1">Lien (optionnel)</label>
        <input type="url" name="link" value="${item.link || ''}" class="w-full px-3 py-2 border rounded" placeholder="https://...">
      </div>
...
async function submitForm(event, type, isEdit) {
  ...
  try {
    ...
    if (isEdit) {
      ...
      method = 'PUT';
      url = `${API_BASE_URL}/${type === 'article' ? 'articles' : type === 'product' ? 'products' : 'posts'}/${id}`;
    } else {
      method = 'POST';
      url = `${API_BASE_URL}/${type === 'article' ? 'articles' : type === 'product' ? 'products' : 'posts'}`;
      if (type === 'ad') formData.append('type', 'ad');
    }
...
async function deleteItem(type, id) {
  ...
  try {
    const url = `${API_BASE_URL}/${type === 'article' ? 'articles' : type === 'product' ? 'products' : 'posts'}/${id}`;
...
function showToast(message, type = 'info') {
  ...
  toast.className = `fixed top-4 right-4 ${colors[type]} text-white px-6 py-3 rounded-lg shadow-lg z-50 max-w-sm`;
