...
async function loadComments(status = 'all') {
    try {
        ...
        let response = await fetch('http://localhost:5000/api/admin/comments', {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
                ...
                if (articlesResponse.ok) {
                    ...
                    if (articlesData.articles) {
                        for (const article of articlesData.articles) {
                            try {
                                const commentsResponse = await fetch(`http://localhost:5000/api/comments/Article/${article._id}`, {
                                    headers: { 'Authorization': `Bearer ${token}` }
                                });
                ...
                if (postsResponse.ok) {
                    ...
                    if (postsData.posts) {
                        for (const post of postsData.posts) {
                            try {
                                const commentsResponse = await fetch(`http://localhost:5000/api/comments/Post/${post._id}`, {
                                    headers: { 'Authorization': `Bearer ${token}` }
                                });
                                if (commentsResponse.ok) {
                                    const commentsData = await commentsResponse.json();
